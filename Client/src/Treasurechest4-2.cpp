...
CTreasurechest4B::CTreasurechest4B(eObjectType _eObjectType)
:Cetc(_eObjectType)
, m_fOpen(0.f)
, m_bOpen(false)
, m_bControl(false)
...
CTreasurechest4B::~CTreasurechest4B()
{
	Release();
}

void CTreasurechest4B::SetDirection(void)
{
}

CTreasurechest4B* CTreasurechest4B::Create(eObjectType _eObjectType, D3DXVECTOR3 _vPos, int iFrame)
{
	CTreasurechest4B*		pTreasurechest4B = new CTreasurechest4B(_eObjectType);

	pTreasurechest4B->Initialize(_vPos, iFrame);

	return pTreasurechest4B;
}

HRESULT CTreasurechest4B::Initialize(void)
{
	return S_OK;
}

HRESULT CTreasurechest4B::Initialize(D3DXVECTOR3 _vPos, int iFrame)
{
...
int CTreasurechest4B::Update(void)
{
	...
	{
		if (!ENGINE::GetEventManager()->IsSwitchOn(L"stage4_box_b_open"))
		{
			m_pMouseCollision->PickObject(m_pOBBCollision->GetOBBInfo(), &m_pInfo->m_matWorld, new CColSwitchOnBill(L"stage4_talk_21"));
		}
		else
		{
			m_pMouseCollision->PickObject(m_pOBBCollision->GetOBBInfo(), &m_pInfo->m_matWorld, new CColSwitchOnBill(L"stage4_talk_22"));
		}
	}
...
void CTreasurechest4B::Render(void)
{
...
void CTreasurechest4B::Release(void)
{
}

HRESULT CTreasurechest4B::AddComponent(void)
{
...
void CTreasurechest4B::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
}
