...
CTreasurechest::CTreasurechest(eObjectType _eObjectType)
:Cetc(_eObjectType)
, m_fOpen(0.f)
, m_bOpen(false)
, m_bControl(false)
...
void CTreasurechest::SetDirection(void)
{
}

CTreasurechest* CTreasurechest::Create(eObjectType _eObjectType, D3DXVECTOR3 _vPos, int iFrame)
{
	CTreasurechest*		pTreasurechest = new CTreasurechest(_eObjectType);

	pTreasurechest->Initialize(_vPos, iFrame);

	return pTreasurechest;
}

HRESULT CTreasurechest::Initialize(void)
{
	return S_OK;
}

HRESULT CTreasurechest::Initialize(D3DXVECTOR3 _vPos, int iFrame)
{
...
int CTreasurechest::Update(void)
{
	...
	{
		if (!ENGINE::GetEventManager()->IsSwitchOn(L"stage2_box_open"))
		{
			m_pMouseCollision->PickObject(m_pOBBCollision->GetOBBInfo(), &m_pInfo->m_matWorld, new CColSwitchOnBill(L"stage2_talk_21"));
		}
		else
		{
			m_pMouseCollision->PickObject(m_pOBBCollision->GetOBBInfo(), &m_pInfo->m_matWorld, new CColSwitchOnBill(L"stage2_talk_22"));
		}
	}
...
void CTreasurechest::Render(void)
{
...
HRESULT CTreasurechest::AddComponent(void)
{
...
void CTreasurechest::SetPos(const D3DXVECTOR3& vPos)
{
	m_pInfo->m_vPos = vPos;
}
